...
fn main() -> Result<()> {
    ...
    println!("   ✓ Retrieved task: task_id={}, status={}", task.task_id, task.status);
    ...
    db.insert_order("ORD-003", "PROD-003", "ACC-002", "completed", 79.99, 1, None)?;
    println!("   ✓ Inserted 3 orders");

    // Get order
    let order = db.get_order("ORD-001")?.expect("Order should exist");
    println!("   ✓ Retrieved order: order_id={}, price=${}", order.order_id, order.price);
    ...
    println!("   ✓ Retrieved session: session_id={}, status={}", session.session_id, session.status);
