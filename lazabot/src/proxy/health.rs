...
impl ProxyHealth {
        ...
        match result {
            Ok(Ok(response)) => {
                if response.status == 200 {
                    debug!("Proxy {}:{} is healthy (status: {})", proxy.host, proxy.port, response.status);
                    true
                } else {
                    warn!("Proxy {}:{} returned non-200 status: {}", proxy.host, proxy.port, response.status);
                    false
                }
            }
            Ok(Err(e)) => {
                warn!("Proxy {}:{} health check failed: {}", proxy.host, proxy.port, e);
                false
            }
            Err(_) => {
                warn!("Proxy {}:{} health check timed out after {:?}", proxy.host, proxy.port, self.timeout_duration);
                false
            }
        }
    }
    ...
    pub async fn check_all_proxies(&self, manager: &ProxyManager) -> Result<()> {
        ...
        info!("Health check completed: {} healthy, {} unhealthy", healthy_count, unhealthy_count);
        Ok(())
    }
    ...
    pub async fn check_healthy_proxies(&self, manager: &ProxyManager) -> Result<()> {
        ...
        info!("Re-checking health of {} currently healthy proxies", healthy_proxies.len());
        ...
        info!("Re-check completed: {} still healthy, {} now unhealthy", still_healthy, now_unhealthy);
        Ok(())
    }
    ...
    pub async fn check_unhealthy_proxies(&self, manager: &ProxyManager) -> Result<()> {
        ...
        info!("Re-checking health of {} currently unhealthy proxies", unhealthy_proxies.len());
        ...
        info!("Recovery check completed: {} now healthy, {} still unhealthy", now_healthy, still_unhealthy);
        Ok(())
    }
    ...
    pub async fn run_comprehensive_check(&self, manager: &ProxyManager) -> Result<HealthReport> {
        let proxies = manager.get_all_proxies();
        info!("Running comprehensive health check for {} proxies", proxies.len());
        ...
        for proxy in proxies {
            ...
            if is_healthy {
                ...
                report.healthy_list.push(format!("{}:{}", proxy.host, proxy.port));
            } else {
                report.unhealthy_proxies += 1;
                report.unhealthy_list.push(format!("{}:{}", proxy.host, proxy.port));
            }
        ...
        info!("Comprehensive health check completed in {:?}", report.check_duration);
        info!("Results: {} healthy, {} unhealthy", report.healthy_proxies, report.unhealthy_proxies);
...
mod tests {
    ...
    async fn test_health_report() {
        let report = HealthReport {
            ...
            healthy_proxies: 3,
            unhealthy_proxies: 1,
            healthy_list: vec!["127.0.0.1:8080".to_string(), "192.168.1.1:3128".to_string(), "10.0.0.1:8080".to_string()],
