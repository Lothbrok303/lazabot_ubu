...
# Lazabot Smoke Test Script
# This script validates the complete pipeline: monitoring -> flash sale detection -> checkout attempt -> database storage
...
LAZABOT_DB="smoke_test.db"
MOCK_SERVER_PID=""
LAZABOT_PID=""
TEST_PRODUCT_ID="smoke-test-product"
TEST_PRODUCT_URL="http://localhost:$MOCK_SERVER_PORT/api/products/$TEST_PRODUCT_ID"
...
cleanup() {
    ...
    if [ ! -z "$LAZABOT_PID" ]; then
        echo "Stopping lazabot (PID: $LAZABOT_PID)"
        kill $LAZABOT_PID 2>/dev/null || true
        wait $LAZABOT_PID 2>/dev/null || true
    fi
    
    # Clean up test database
    if [ -f "$LAZABOT_DB" ]; then
...
# Create mock server
create_mock_server() {
    print_status "Creating mock Lazada API server..."
...
create_test_products() {
    ...
    cat > "$SCRIPT_DIR/test_products.yaml" << EOF
products:
  - id: "$TEST_PRODUCT_ID"
