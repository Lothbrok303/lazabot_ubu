...
async fn test_session_manual_persistence() -> Result<()> {
    ...
    println!("✓ Restored session with {} cookies", restored_session.cookies.len());
    
    // 4. VERIFY: Check integrity
    assert_eq!(session.id, restored_session.id);
    assert_eq!(session.credentials.username, restored_session.credentials.username);
    assert_eq!(session.cookies.len(), restored_session.cookies.len());
    assert_eq!(session.cookies.get("session_id"), restored_session.cookies.get("session_id"));
    assert_eq!(session.cookies.get("auth_token"), restored_session.cookies.get("auth_token"));
    assert_eq!(session.cookies.get("user_id"), restored_session.cookies.get("user_id"));
