...
use lazabot::core::{SessionManager, Session, Credentials};
...
async fn test_session_cleanup() -> Result<()> {
    ...
    let credentials = Credentials::new("cleanup_test_user".to_string(), "cleanup_test_pass".to_string());
    let session = manager.login(credentials).await?;
    manager.persist_session(&session).await?;
    ...
    info!("✓ Session cleanup test successful (cleaned {} sessions)", cleaned_count);
    Ok(())
}
...
async fn test_session_encryption() -> Result<()> {
    ...
    let credentials = Credentials::new("encryption_test_user".to_string(), "sensitive_password".to_string());
    let mut session = manager.login(credentials).await?;
    
    // Add some metadata
    session.add_metadata("sensitive_data".to_string(), serde_json::Value::String("secret_value".to_string()));
    ...
    assert_eq!(restored_session.id, session.id);
    assert_eq!(restored_session.credentials.username, session.credentials.username);
    assert_eq!(restored_session.credentials.password, session.credentials.password);
    assert_eq!(restored_session.cookies.len(), session.cookies.len());
...
async fn test_session_cookie_handling() -> Result<()> {
    ...
    // Create session
    let credentials = Credentials::new("cookie_test_user".to_string(), "cookie_test_pass".to_string());
    let mut session = manager.login(credentials).await?;
    ...
    assert_eq!(restored_session.cookies.len(), session.cookies.len());
    assert_eq!(restored_session.cookies.get("custom_cookie"), Some(&"custom_value".to_string()));
    assert_eq!(restored_session.cookies.get("another_cookie"), Some(&"another_value".to_string()));
...
async fn test_storage_with_task_manager() -> Result<()> {
    ...
    db.update_task_status(task_id, "completed", Some(started_at), Some(completed_at), None)?;
...
async fn test_deployment_setup_scripts() -> Result<()> {
    ...
    assert!(std::path::Path::new(setup_script).exists(), "Setup script should exist");
    
    // Test that deployment script exists
    let deploy_script = "scripts/deploy_remote.sh";
    assert!(std::path::Path::new(deploy_script).exists(), "Deploy script should exist");
    
    // Test that test script exists
    let test_script = "scripts/test_deployment.sh";
    assert!(std::path::Path::new(test_script).exists(), "Test script should exist");
...
async fn test_docker_configuration() -> Result<()> {
    ...
    assert!(std::path::Path::new(compose_file).exists(), "Docker Compose file should exist");
...
async fn test_deployment_documentation() -> Result<()> {
    ...
    assert!(std::path::Path::new(deployment_doc).exists(), "Deployment documentation should exist");
    
    // Test that deploy README exists
    let deploy_readme = "deploy/README.md";
    assert!(std::path::Path::new(deploy_readme).exists(), "Deploy README should exist");
