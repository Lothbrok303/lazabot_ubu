...
async fn test_mock_recaptcha_solving() {
    ...
    let result = solver
        .solve_recaptcha("test_site_key", "https://example.com")
        .await
        .unwrap();
...
async fn test_captcha_solver_from_env_missing() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("CAPTCHA_API_KEY environment variable not set"));
}
...
async fn test_mock_solver_trait_implementation() {
    let mock_solver =
        MockCaptchaSolver::new("mock_image".to_string(), "mock_recaptcha".to_string());
